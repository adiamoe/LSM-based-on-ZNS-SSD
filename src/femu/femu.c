...
static void dispatch_io(struct FemuCtrl *femu, uint64_t len, uint64_t offset, void *arg, bool wait, int op)
{
...
{
    dispatch_io(femu, len, offset, arg, wait, NVME_CMD_READ);
...
{
    dispatch_io(femu, len, offset, arg, wait, NVME_CMD_WRITE);
...
int femu_reset(struct FemuCtrl *femu, uint64_t len, uint64_t offset, void *arg) {
    ...
    dispatch_io(femu, len, offset, arg, false, NVME_CMD_RESET);
